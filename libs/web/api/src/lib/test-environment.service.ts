...
import { TestEnvironment, TestEnvironmentDto } from './test-environment.interface';

@Injectable()
export class TestEnvironmentService {
  constructor(@Inject('ENV_CONF') private environment, private http: HttpClient) {}

  create(testDto: TestEnvironmentDto): Observable<TestEnvironment> {
  ...
  updateOne(id: string, testDto: TestEnvironmentDto): Observable<TestEnvironment> {
  ...
  startTestEnvironment(id: string, testDto: TestEnvironment): Observable<TestEnvironment> {
